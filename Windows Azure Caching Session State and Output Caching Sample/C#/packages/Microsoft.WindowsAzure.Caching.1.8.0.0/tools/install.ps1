...
foreach ($configFile in $configFiles)
{
    ...
    # Config update 1 of 3: Remove redundant autoDiscovery elements
    ...
    {
        $redundantNodes = $defaultCacheClientNode.SelectNodes('autoDiscover[@identifier="[cache cluster role name]"]')
    ...
    # Config update 2 of 3: Remove redundant crashDump elements
    ...
    # Config update 3 of 3: Get traceLevel to be later mapped to clientDiagnosticsLevel
...
{
    $cscfgXml.Save($cscfgXmlTable.Get_Item($cscfgXml))
}
